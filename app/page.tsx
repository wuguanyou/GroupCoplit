...
export default function Home() {
  ...
  const report = (task?: Task) => {
    const t = task ?? p?.tasks.find((t) => t.status !== 'done' && t.owner);
    if (t)
      open('report', {
        taskId: t.id,
        memberId: t.owner,
  ...
  const evidence = (task?: Task) => {
    ...
    if (t)
      open('evidence', {
        taskId: t.id,
        memberId: t.owner || p?.members[0].id,
        kind: 'delivery',
        hours: 1,
        note: '',
        url: '',
    ...
    <div className="shell">
      <aside>
        ...
        <div className="sideproject">
          <span className="dot" />
          進行中的專案<strong>{p?.name ?? '校園 AI 系統'}</strong>
      ...
      <main>
        <header>
          <span>
            工作空間 <ChevronRight size={14} /> {p?.name ?? '校園 AI 系統'}
          </span>
          <div className="header-right">
            <span className="badge neutral">
              {p?.demo ? '示範資料' : '專案'}
            </span>
            <span className="avatar small">冠</span>
          </div>
        </header>
        {error && (
              ...
              <>
                <div className="notice">
                  ...
                  <div>
                    ...
                    <p>
                      分數來自交叉驗收紀錄，自報工時不直接等於貢獻。示範版的組員身分由操作者選擇。
                    </p>
                  </div>
            ...
            >
              ...
              <form onSubmit={submit}>
                ...
                {modal === 'review' && (
                  <>
                    <p>
                      請檢查成果是否符合任務完成標準。示範工作空間中，你可以選擇代表哪位組員驗收。
                    </p>
                    <label>
                      驗收組員
                      <select
